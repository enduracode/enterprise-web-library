...
- **Use automatic database connection management by default throughout application code, including services and console apps.** Use the existing manager in framework-managed contexts such as web requests; establish an automatic scope for standalone operations. Keep reads in the default cached phase and enable modifications narrowly for database writes or calls that queue external effects such as email. Service code using `ExecuteWithConnectionOpen` and `ExecuteInTransaction` is legacy, not a pattern to copy. Require a concrete reason for manual connection/transaction management. See `ewl-data-access` for scope boundaries and the deferred-email pattern.
