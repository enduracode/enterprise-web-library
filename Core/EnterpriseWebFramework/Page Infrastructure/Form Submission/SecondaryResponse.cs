...
public class SecondaryResponse {
	...
	public SecondaryResponse( BlobFileResponse response, bool useMemoryCache ): this(
		response.GetResponse,
		memoryCachingSetup: useMemoryCache ? new ResponseMemoryCachingSetup( response.MemoryCacheKey, response.FileLastModificationTime ) : null ) {}
	...
	public SecondaryResponse( Func<EwfResponse> responseCreator, ResponseMemoryCachingSetup? memoryCachingSetup = null ) {
		fullResponseGetter = () =>
			memoryCachingSetup is not null
				? FullResponse.GetFromCache( memoryCachingSetup.CacheKey, memoryCachingSetup.LastModificationTime, () => responseCreator().CreateFullResponse() )
				: responseCreator().CreateFullResponse();
	}

	internal FullResponse GetFullResponse() => fullResponseGetter();
}