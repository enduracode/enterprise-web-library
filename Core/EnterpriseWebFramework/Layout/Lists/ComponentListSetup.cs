...
public class ComponentListSetup {
	...
	public ComponentListSetup(
		...
		componentGetter = ( listTypeClasses, items ) => {
			...
			return new IdentifiedFlowComponent( () => new IdentifiedComponentData<FlowComponentOrNode>(
				"",
				new[]
					{
						new UpdateRegionLinker(
							"tail",
							from region in tailUpdateRegions?.Collection.Value ?? [ ]
						...
						new UpdateRegionLinker(
							"remove",
							items.Select( ( item, index ) => new PreModificationUpdateRegion(
				...
				errorsBySource => new DisplayableElement( context => {
					return new DisplayableElementData(
						...
						children: itemComponents.Select( ( component, index ) =>
								index == itemComponents.Length - 1 && lastItemAutofocusCondition != null
									? new FlowAutofocusRegion( lastItemAutofocusCondition, new CustomFlowComponent( component.ToCollection() ).ToCollection() )
									: component )
							.Materialize(),
