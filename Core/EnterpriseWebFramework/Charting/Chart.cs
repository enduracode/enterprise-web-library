...
public sealed class Chart: FlowComponent {
	private static readonly ElementClass elementClass = new( "ewfChart" );
		...
		switch( setup.ChartType ) {
			...
			case ChartType.HorizontalStackedBar:
				...
				var xAxis = new JObject(
					...
					new JProperty( "scaleLabel", new JObject( new JProperty( "display", setup.XAxisTitle.Any() ), new JProperty( "labelString", setup.XAxisTitle ) ) ) );
			...
			new ElementComponent( context => new ElementData( () => {
				var jsInitStatement = "new Chart( '{0}', {{ type: '{1}', data: {2}, options: {3} }} );".FormatWith(
					...
					new JObject(
						...
						new JProperty( "datasets", new JArray( dataSets.Zip( colors, ( set, color ) => datasetSelector( set, color ) ) ) ) ).ToString( Formatting.None ),
							            ...
							            ColumnPrimaryTable.Create( postBackIdBase: setup.PostBackIdBase, allowExportToExcel: true, firstDataFieldIndex: 1 )
								            .AddItems(
									            ( setup.XAxisTitle.Any() || setup.Labels.Any( i => i.Any() )
										            ...
										            dataSets.Select( dataSet => EwfTableItem.Create(
