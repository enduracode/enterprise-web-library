...
public static class GenericPhrasingContainerExtensionCreators {
	...
	/// </summary>
	public static PhrasingComponent ToGenericPhrasingContainer<ModificationValueType>(
		...
		return new CustomPhrasingComponent(
			new DisplayableElement( context => {
				pageModificationValue.AddJsModificationStatement( valueExpression =>
					"$( '#{0}' ).text( {1} );".FormatWith( context.Id, jsTextExpressionGetter( valueExpression ) ) );
