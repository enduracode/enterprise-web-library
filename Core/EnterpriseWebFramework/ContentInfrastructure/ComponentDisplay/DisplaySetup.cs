	...
	public static DisplaySetup ToDisplaySetup( this PageModificationValueCondition pageModificationValueCondition ) =>
		new(
			Tuple.Create<Action<string>, Action<string>>(
				statements =>
					pageModificationValueCondition.AddJsModificationStatement( expression => "if( {0} )".FormatWith( expression ) + " { " + statements + " }" ),
				statements => pageModificationValueCondition.AddJsModificationStatement( expression =>
					"if( !( {0} ) )".FormatWith( expression ) + " { " + statements + " }" ) ),
