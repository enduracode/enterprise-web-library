...
internal class CheckboxListCssElementCreator: ControlCssElementCreator {
	internal static readonly ElementClass ListClass = new( "ewfCheckboxList" );
	internal static readonly ElementClass ActionContainerClass = new( "ewfClA" );
...
public sealed class CheckboxList<ItemIdType>: FormControl<FlowComponent> {
	...
	public CheckboxList(
		...
		var checkboxes = setup.Items.Select( i => new FlowCheckbox(
				  ...
				  ? new GenericFlowContainer(
					  new InlineList(
						  new EwfButton(
								  ...
								  behavior:
								  new CustomButtonBehavior( () => "$( '#{0}' ).find( 'input[type=checkbox]:not(:checked)' ).click();".FormatWith( contentContainerId.Id ) ) )
							  .ToCollection()
							  .ToComponentListItem()
							  .ToCollection()
							  .Append(
								  new EwfButton(
										  ...
										  behavior:
										  new CustomButtonBehavior( () => "$( '#{0}' ).find( 'input[type=checkbox]:checked' ).click();".FormatWith( contentContainerId.Id ) ) )
									  .ToCollection()
