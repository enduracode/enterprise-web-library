...
public class TextControl: FormControl<PhrasingComponent> {
	...
	public TextControl(
		string value, bool allowEmpty, TextControlSetup? setup = null, int? minLength = null, int? maxLength = null,
		Action<string, Validator>? validationMethod = null ) {
		setup ??= TextControlSetup.Create();
		( Labeler, PageComponent, Validation ) = setup.LabelerAndComponentAndValidationGetter(
			value,
			allowEmpty,
			minLength,
			maxLength,
			( postBackValue, _ ) => postBackValue,
			validationMethod );
	}
}