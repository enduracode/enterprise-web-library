	...
	public class Checkbox: FormControl<PhrasingComponent> {
		...
		private class CssElementCreator: ControlCssElementCreator {
			IReadOnlyCollection<CssElement> ControlCssElementCreator.CreateCssElements() =>
				new CssElement( "Checkbox", "span.{0}".FormatWith( elementClass.ClassName ) ).ToCollection()
					.Append( new CssElement( "CheckboxControl", "input.{0}".FormatWith( elementClass.ClassName ) ) )
					.Append( new CssElement( "CheckboxLabel", "label.{0}".FormatWith( elementClass.ClassName ) ) )
					.Materialize();
		}
			...
			return new GenericPhrasingContainer(
				new CustomPhrasingComponent(
						new DisplayableElement(
							...
							formValue: formValue ).ToCollection() ).ToCollection()
					.Concat( label.Any() ? labeler.CreateLabel( label, classes: elementClass ) : Enumerable.Empty<PhrasingComponent>() )
					.Materialize(),
