	...
	public class InlineUpdate: InlineDbModificationCommand, InlineDbCommandWithConditions {
		...
		public int Execute( DBConnection cn ) {
			...
			foreach( var columnMod in columnModifications ) {
				columnMod.Parameter.Name = GetParamNameFromNumber( paramNumber++ );
				command.CommandText += columnMod.ColumnName + " = " + columnMod.Parameter.GetNameForCommandText( cn.DatabaseInfo ) + ", ";
				command.Parameters.Add( columnMod.Parameter.GetAdoDotNetParameter( cn.DatabaseInfo ) );
			}
