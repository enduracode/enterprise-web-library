...
namespace RedStapler.TestWebSite.TestPages {
	public partial class ToolTipTest: EwfPage {
		...
		protected override void LoadData( DBConnection cn ) {
			...
			ph.AddControlsReturnThis( new Box( toolTipLink ) );
			ph.AddControlsReturnThis(
				new Box(
					new ToolTipLink( new EwfLink( new ExternalPageInfo( "http://www.google.com" ) ) { ActionControlStyle = new TextActionControlStyle( "Google link" ) } )
						{ ActionControlStyle = new TextActionControlStyle( "Not clickable 2" ) } ) );

			label.ToolTip = "Label tool tip";

			extra.Controls.Add( new ToolTipLink( EwfLink.Create( new ExternalPageInfo( "http://redstapler.biz" ), new TextActionControlStyle( "Testing Controls" ) ) )
			...
			calendarTest.AddContentForDay( new DateTime( DateTime.Today.Year, DateTime.Today.Month, 9 ),
			                               new ToolTipLink( "Testing clickable link inside of a calendar".GetLiteralControl() )
