...
internal static class RetrievalStatics {
	internal static void WriteRowClasses(
		...
		writer.WriteLine( "return " + EwlStatics.GetCSharpIdentifier( columns.First( c => c.UseToUniquelyIdentifyRow ).PascalCasedName ) + ".GetHashCode();" );
		...
		foreach( var column in columns.Where( c => c.UseToUniquelyIdentifyRow ) )
			condition = StringTools.ConcatenateWithDelimiter(
				" && ",
				condition,
				EwlStatics.GetCSharpIdentifier( column.PascalCasedName ) + " == other." + EwlStatics.GetCSharpIdentifier( column.PascalCasedName ) );
	...
	private static void writeColumnProperty( TextWriter writer, Column column ) {
		writeColumnComment( writer, column );
		writer.WriteLine(
			"public " + column.DataTypeName + " " + EwlStatics.GetCSharpIdentifier( column.PascalCasedName ) + " => __basicRow." +
