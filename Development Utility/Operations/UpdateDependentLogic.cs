...
internal class UpdateDependentLogic: Operation {
	...
	void Operation.Execute( Installation genericInstallation, IReadOnlyList<string> arguments, OperationResult operationResult ) {
		...
		using( var writer = new StreamWriter( EwlStatics.CombinePaths( installation.GeneralLogic.Path, "Directory.Build.props" ), false, Encoding.UTF8 ) ) {
			...
			// see https://learn.microsoft.com/en-us/dotnet/core/project-sdk/msbuild-props#framework-properties
		...
		if( !installation.DevelopmentInstallationLogic.SystemIsEwl ) {
			using var writer = new StreamWriter(
				EwlStatics.CombinePaths( installation.DevelopmentInstallationLogic.LibraryPath, "Directory.Build.targets" ),
				false,
				Encoding.UTF8 );
			writer.WriteLine( $"<!-- generated by {EwlStatics.EwlInitialism} -->" );
			writer.WriteLine( "<Project>" );
			var condition = installation.ExistingInstallationLogic.RuntimeConfiguration.SystemUsesLegacyEwl.HasValue
				                ? """
				                   Condition="'$(MSBuildProjectName)'=='Library New'"
				                  """
				                : "";
			writer.WriteLine( $"""<Target Name="{EwlStatics.EwlInitialism.EnglishToPascal()}ToolRestore" BeforeTargets="CollectPackageReferences"{condition}>""" );
			writer.WriteLine( """<Exec Command="dotnet tool restore" WorkingDirectory="$(MSBuildThisFileDirectory).." />""" );
	...
	private void generateUnitTestProjectCode( DevelopmentInstallation installation ) {
		...
		// Use a runtime identifier because this project ends up compiling like a console app (due to the Microsoft.NET.Test.Sdk dependency above) rather than a
		// class library.
		generateCodeForProject(
			installation,
	...
	private void generateCodeForProject(
		...
		using( var writer = new StreamWriter( EwlStatics.CombinePaths( projectPath, "Directory.Build.props" ), false, Encoding.UTF8 ) ) {
			...
			// Include the first Directory.Build.props file found in a parent directory, which should be the one we generate at the installation level.
			writer.WriteLine( """<Import Project="$([MSBuild]::GetPathOfFileAbove('$(MSBuildThisFile)','$(MSBuildThisFileDirectory)../'))" />""" );
			...
			if( !installation.DevelopmentInstallationLogic.SystemIsEwl && !installation.SystemIsTewl() )
				if( projectName.Equals( "Library", StringComparison.Ordinal ) || projectName.Equals( IsuStatics.DataMigratorProjectName, StringComparison.Ordinal ) ) {
					const string toolPackageSuffix = ".DevelopmentUtility";
					var toolMember = JsonNode.Parse( File.ReadAllText( EwlStatics.CombinePaths( installation.GeneralLogic.Path, "dotnet-tools.json" ) ) )![ "tools" ]!
						.AsObject()
						.Single( i => i.Key.EndsWith( toolPackageSuffix, StringComparison.OrdinalIgnoreCase ) );
					var mainPackageId = toolMember.Key[ ..^toolPackageSuffix.Length ];
					var version = toolMember.Value![ "version" ]!.GetValue<string>();
					foreach( var id in mainPackageId.ToCollection()
						        .Concat( installation.DevelopmentInstallationLogic.DevelopmentConfiguration.Providers?.Select( i => $"{mainPackageId}.{i}" ) ?? [ ] ) )
						writer.WriteLine( $"""<PackageReference Include="{id}" Version="{version}" />""" );
				}
	...
	private void updateOpenCodeConfig( DevelopmentInstallation installation ) {
		var nugetGlobalPackagesPattern = EwlStatics.CombinePaths(
			SettingsUtility.GetGlobalPackagesFolder( Settings.LoadDefaultSettings( installation.GeneralLogic.Path ) ),
			"**" );
		var instructionsPath = installation.DevelopmentInstallationLogic.SystemIsEwl
			                       ? ""
			                       : $"""
			                          "Library/{generatedCodeFolderName}/{EwlStatics.EwlInitialism} Agent Rules.md"
			                          """;
		File.WriteAllText(
			EwlStatics.CombinePaths( installation.GeneralLogic.Path, "opencode.jsonc" ),
			$$"""
			  // generated by {{EwlStatics.EwlInitialism}}
			  {
			    ...
			    "permission": {
			      "external_directory": { "{{nugetGlobalPackagesPattern}}": "allow" },
			      "edit": { "{{nugetGlobalPackagesPattern}}": "deny" }
			    },
