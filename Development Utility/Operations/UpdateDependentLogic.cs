	...
	void Operation.Execute( Installation genericInstallation, IReadOnlyList<string> arguments, OperationResult operationResult ) {
		...
		if( installation.DevelopmentInstallationLogic.SystemIsEwl ) {
			...
			generateCodeForProject(
				...
				_ => {},
				runtimeIdentifier: platformRuntimeIdentifier );
			generateCodeForProject(
				installation,
				"MySQL Provider",
		...
		// XML schema generation requires Windows SDK tools (xsd.exe, SvcUtil.exe)
		if( Environment.OSVersion.Platform == PlatformID.Win32NT ) {
			...
			Log.Information( "Skipping XML schema generation (Windows SDK tools not available on this platform)." );
