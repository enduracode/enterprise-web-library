...
The build server (EWL System Manager ISU) builds EWL using only the
**released DU** from the latest published NuGet package:

1. Clone the repository and `dotnet restore`.
2. Download the latest `Ewl` NuGet package and run its DU with
   `UpdateDependentLogic`.
3. `dotnet restore --force` (picks up regenerated `Directory.Build.props`).
4. Run the released DU again with `ExportLogic` (builds and packages).

All code generation and building is done by the DU from the latest published
package. This means any changes pushed to EWL must compile against code
generated by the currently released DU. Changes to code generation logic
typically need to be pushed and published first, then changes that depend
on the new generation can be pushed afterward.
...
1. **Before editing any C# and XML/XSD files, you MUST invoke the `ewl-cleanup` subagent first.**
