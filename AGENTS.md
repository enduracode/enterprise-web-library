...
3. **Never edit files in any `Generated Code\` folder or the `.opencode\` folder.** They
   ...
   in the `.opencode\` output folder. After doing so, follow rule 4 below.
4. **After changing DU code or any source files that feed into code generation**
...
6. Configuration lives in XML files validated against XSD schemas in `Configuration\` folders.
7. UI is built with EWL's component model (methods returning component collections), not Razor.
8. Page classes use EWL's code-generation-based URL routing, inheriting from generated bases.
