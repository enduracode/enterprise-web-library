...
3. **Never edit files in any `Generated Code\` folder, the `.opencode\` folder,
   EWL-prefixed skill directories under `.agents\skills\`, the `.claude\` folder, or `.mcp.json`.** They are fully regenerated by the
   Development Utility. Your changes will be overwritten. See the `sync`
   operation in the DU for how these are generated.
