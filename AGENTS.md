...
3. **Never edit files in any `Generated Code\` folder, the `.opencode\` folder,
   the `.claude\` folder, or `.mcp.json`.** They are fully regenerated by the
   Development Utility. Your changes will be overwritten. See the
   `UpdateDependentLogic` operation in the DU for how these are generated.
   After changing source files, follow rule 4 below.
